...
(function (global) {
  ...
  RAW.forEach(function (s) {
    var store = {
      /* STRUCTURED OPENING HOURS — the single source of a store's hours:
         the closing time, the 30-minute last-order rule and every hours text
         customers see (generated by RAFStoreSchedule, which alone writes it).
         ...
         Times are 24-hour 'HH:MM', one or two periods a day. An earlier
         single-period day { open, close } still reads. The free-text `hours`
         field below is LEGACY data only: never parsed, edited or shown as
         hours. No store is seeded with a schedule: an absent one reports a
         genuine "not configured" state rather than a guessed time. */
